...
# ファイル名では center1.0 ではなく center1 を使用する
...
if vq_min < 0 or vq_max >= int(centers.shape[0]):
    raise ValueError(
        f"VQ ID out of range: {vq_min}..{vq_max}"
    )

actual_scale = float(
    codebook.get("args", {}).get(
        "center_scale",
        -1.0,
    )
)
